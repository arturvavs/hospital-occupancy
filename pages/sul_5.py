...
dash.register_page(__name__, path='/5sul')
...
layout = html.Div(children=[
    html.Div([
        ...
        html.H2('5º SUL', className='subtitulo')
    ], className='cabecalho'),
    ...
    html.Div([
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-5sul', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-pacientes-5sul', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-livres-5sul', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-higienizacao-5sul', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-reservados-5sul', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
...
#ARMAZENAMENTO DE INFORMAÇÃO SOBRE PAGINA DA TABELA
        dcc.Store(id='page-store-5sul', data={'current_page': 0}),
        dcc.Store(id='dataset-5sul'),
...
@callback(
    Output('total-leitos-5sul', 'children'),
    Output('total-pacientes-5sul', 'children'),
    Output('total-leitos-livres-5sul', 'children'),
    Output('total-leitos-higienizacao-5sul', 'children'),
    Output('total-leitos-reservados-5sul', 'children'),
    Output('dataset-5sul','data'),
    Input('interval-component-5sul','n_intervals')
)
def update_cards(n_intervals):
    df_pacientes = database.get_data(sql)
    filter_df_pacientes = df_pacientes[df_pacientes['SETOR_REDUZIDO'] == '5º SUL']
...
@callback(
    Output('table-data-5sul', 'data'), #Callback para atualizar a tabela
    Output('table-data-5sul', 'page_current'), #Callback para atualizar a página da tabela
    Input('interval-component-5sul', 'n_intervals'), #Callback que obtem do dcc.Interval o valor n_intervals para atualização da página
    Input('dataset-5sul','data'),
    State('page-store-5sul', 'data') #Callback para identificar qual a página atual da tabela
)
...
@callback(
    Output('page-store-5sul', 'data'),
    Input('interval-component-5sul', 'n_intervals'),
    State('page-store-5sul', 'data')
)

...
dash.register_page(__name__, path='/7norte')
...
layout = html.Div(children=[
    html.Div([
        ...
        html.H2('7º NORTE', className='subtitulo')
    ], className='cabecalho'),
    ...
    html.Div([
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-7norte', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-pacientes-7norte', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-livres-7norte', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-higienizacao-7norte', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div(id='total-leitos-reservados-7norte', className='text-center', style={"font-size": "50px", "font-family": "Trebuchet MS", "line-height": "2.0"})
            ])
...
#ARMAZENAMENTO DE INFORMAÇÃO SOBRE PAGINA DA TABELA
        dcc.Store(id='page-store-7norte', data={'current_page': 0}),
        dcc.Store(id='dataset-7norte'),
    
    html.Div([
        dcc.Link(href='/7norte')
    ], className='link')
],id='layout_p4')

# Callback para atualizar os valores dos cartões
@callback(
    Output('total-leitos-7norte', 'children'),
    Output('total-pacientes-7norte', 'children'),
    Output('total-leitos-livres-7norte', 'children'),
    Output('total-leitos-higienizacao-7norte', 'children'),
    Output('total-leitos-reservados-7norte', 'children'),
    Output('dataset-7norte','data'),
    Input('interval-component-7norte','n_intervals')
)
def update_cards(n_intervals):
    df_pacientes = database.get_data(sql)
    filter_df_pacientes = df_pacientes[df_pacientes['SETOR_REDUZIDO'] == '7º NORTE']
...
@callback(
    Output('table-data-7norte', 'data'), #Callback para atualizar a tabela
    Output('table-data-7norte', 'page_current'), #Callback para atualizar a página da tabela
    Input('interval-component-7norte', 'n_intervals'), #Callback que obtem do dcc.Interval o valor n_intervals para atualização da página
    Input('dataset-7norte','data'),
    State('page-store-7norte', 'data') #Callback para identificar qual a página atual da tabela
)
...
@callback(
    Output('page-store-7norte', 'data'),
    Input('interval-component-7norte', 'n_intervals'),
    State('page-store-7norte', 'data')
)
